...
bool BlockchainExplorer::addObserver(IBlockchainObserver* observer) {
  if (state.load() != INITIALIZED) {
    throw std::system_error(make_error_code(MevaCoin::error::BlockchainExplorerErrorCodes::NOT_INITIALIZED));
  }
...
void BlockchainExplorer::init() {
  if (state.load() != NOT_INITIALIZED) {
    ...
    throw std::system_error(make_error_code(MevaCoin::error::BlockchainExplorerErrorCodes::ALREADY_INITIALIZED));
  }
  if (node.addObserver(this)) {
    ...
    throw std::system_error(make_error_code(MevaCoin::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
  }
...
uint64_t BlockchainExplorer::getRewardBlocksWindow() {
  ...
  return parameters::MEVACOIN_REWARD_BLOCKS_WINDOW;
}
...
uint64_t BlockchainExplorer::getFullRewardMaxBlockSize(uint8_t majorVersion) {
  ...
  if (majorVersion >= BLOCK_MAJOR_VERSION_3) {
    return parameters::MEVACOIN_BLOCK_GRANTED_FULL_REWARD_ZONE;
  } else if (majorVersion == BLOCK_MAJOR_VERSION_2) {
    return parameters::MEVACOIN_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
  } else {
    return parameters::MEVACOIN_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
  }
}
