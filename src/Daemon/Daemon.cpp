...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file               = { "config-file", "Specify configuration file", std::string(MevaCoin::MEVACOIN_NAME) + ".conf" };
  ...
  bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
    ...
    if (command_line::get_arg(vm, command_line::arg_version)) {
      std::cout << MevaCoin::MEVACOIN_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
      exit = true;
    }
  ...
  void print_genesis_tx_hex(const po::variables_map& vm, LoggerManager& logManager) {
    MevaCoin::Transaction tx = MevaCoin::CurrencyBuilder(logManager).generateGenesisTransaction();
    std::string tx_hex = Common::toHex(MevaCoin::toBinaryArray(tx));
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      {
        std::cout << MevaCoin::MEVACOIN_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
    ...
    logger(INFO) << MevaCoin::MEVACOIN_NAME << " v. " << PROJECT_VERSION_LONG;
...
Common::Console::Color::BrightCyan);
    ...
    try {
      ...
      std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Please launch: " << MevaCoin::MEVACOIN_NAME << "d --" << arg_print_genesis_tx.name;
      return 1;
    }
    ...
    MevaCoin::Core m_core(currency, nullptr, logManager, dispatcher, vm["enable-blockchain-indexes"].as<bool>(), allow_reorg, no_blobs);

    bool disable_checkpoints = command_line::get_arg(vm, arg_disable_checkpoints);
    if (!disable_checkpoints) {
      MevaCoin::Checkpoints checkpoints(logManager, allow_reorg);
    ...
    MevaCoin::MevaCoinProtocolHandler cprotocol(currency, dispatcher, m_core, nullptr, logManager);
    MevaCoin::NodeServer p2psrv(dispatcher, cprotocol, logManager);
    MevaCoin::RpcServer rpcServer(rpcConfig, dispatcher, logManager, m_core, p2psrv, cprotocol);

    cprotocol.set_p2p_endpoint(&p2psrv);
    m_core.set_mevacoin_protocol(&cprotocol);
    ...
    m_core.set_mevacoin_protocol(NULL);
