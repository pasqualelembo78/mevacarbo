...
void printPrivateKeys(MevaCoin::WalletGreen &wallet, bool viewWallet)
{
    ...
    MevaCoin::AccountBase::generateViewFromSpend(privateSpendKey,
...
void printHashrate(uint64_t difficulty)
{
    ...
    uint32_t hashrate = static_cast<uint32_t>(
        round(difficulty / MevaCoin::parameters::DIFFICULTY_TARGET)
    );
...
void saveCSV(MevaCoin::WalletGreen &wallet, MevaCoin::INode &node)
{
    ...
    {
        const MevaCoin::WalletTransaction t = wallet.getTransaction(i);
...
void txProof(MevaCoin::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        if (!MevaCoin::parseAccountAddressString(prefix, destAddress, addrStr))
        {
...
void verifyMessage(MevaCoin::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        if (!MevaCoin::parseAccountAddressString(prefix, address, addrStr))
        {
            std::cout << WarningMsg("Failed to parse address") << std::endl;
        }
        else
