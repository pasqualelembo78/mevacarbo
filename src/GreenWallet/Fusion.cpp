                             ...
                             uint64_t threshold)
{
    ...
    if (optimizable == 0)
    {
        return MevaCoin::WALLET_INVALID_TRANSACTION_ID;
    }
...
bool optimize(MevaCoin::WalletGreen &wallet, uint64_t threshold)
{
    ...
    while (true)
    {
        ...
        if (tmpFusionTxID == MevaCoin::WALLET_INVALID_TRANSACTION_ID)
        {
            break;
        }
        else
        {
            const MevaCoin::WalletTransaction w
    ...
    while (true)
    {
        const std::vector<MevaCoin::WalletTransactionWithTransfers>
