...
std::string getPaymentIDFromExtra(std::string extra)
{
	...
	if (extra.length() > 0)
	{
		...
		if (MevaCoin::getPaymentIdFromTxExtra(vecExtra, paymentIdHash))
		{
...
uint64_t calculateNodeFee(uint64_t amount) {
  ...
  if (node_fee > (uint64_t)MevaCoin::parameters::COIN)
      node_fee = (uint64_t)MevaCoin::parameters::COIN;
  return node_fee;
}
...
bool shutdown(std::shared_ptr<WalletInfo> walletInfo, MevaCoin::INode &node,
