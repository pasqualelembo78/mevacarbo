...
bool confirmTransaction(MevaCoin::TransactionParameters t,
...
void sendMultipleTransactions(MevaCoin::WalletGreen &wallet,
                              std::vector<MevaCoin::TransactionParameters>
                              transfers)
{
    ...
    for (auto tx : transfers)
    {
        while (true)
        {
            ...
            if (neededBalance < wallet.getActualBalance())
            {
                ...
                const MevaCoin::WalletTransaction sentTx
...
void splitTx(MevaCoin::WalletGreen &wallet,
             MevaCoin::TransactionParameters p)
{
    ...
    MevaCoin::TransactionParameters restoreInitialTx = p;
    ...
    for (int numTxMultiplier = 1; ; numTxMultiplier++)
    {
        ...
        std::vector<MevaCoin::TransactionParameters> transfers;

        for (int i = 0; i < numTransactions; i++)
        {
            MevaCoin::TransactionParameters tmp = p;
...
void transfer(std::shared_ptr<WalletInfo> walletInfo, uint32_t height, bool sendAll, std::string nodeAddress, uint64_t nodeFee)
{
    ...
    if (!nodeAddress.empty() && nodeFee == 0)
      ...
      nodeFee = std::min<uint64_t>(nodeFee, (uint64_t)MevaCoin::parameters::COIN);
                ...
                std::string nodeAddress, uint64_t nodeFee)
{
    ...
    p.destinations = std::vector<MevaCoin::WalletOrder>
    {
        {address, amount}
    };
    ...
    while (true)
    {
        try
        {
            if (walletInfo->wallet.txIsTooLarge(p))
            {
                ...
                else
                {
                    ...
                    const MevaCoin::WalletTransaction tx
        ...
        catch (const std::system_error &e)
        {
            ...
            switch (e.code().value())
            {
                case WalletErrors::MevaCoin::error::WRONG_AMOUNT:
                {
                    wrongAmount = true;
                ...
                case WalletErrors::MevaCoin::error::MIXIN_COUNT_TOO_BIG:
                case NodeErrors::MevaCoin::error::INTERNAL_NODE_ERROR:
                {
                ...
                case NodeErrors::MevaCoin::error::NETWORK_ERROR:
                case NodeErrors::MevaCoin::error::CONNECT_ERROR:
                {
...
Maybe<std::string> getPaymentID(std::string msg)
{
    while (true)
    {
        ...
        if (!MevaCoin::createTxExtraWithPaymentId(paymentID, extra))
        {
...
std::string getExtraFromPaymentID(std::string paymentID)
{
    ...
    /* Convert the payment ID into an "extra" */
    MevaCoin::createTxExtraWithPaymentId(paymentID, extra);
...
bool parseAddress(std::string address)
{
    ...
    const bool valid = MevaCoin::parseAccountAddressString(prefix, addr,
                                                             address);
