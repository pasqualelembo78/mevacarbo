...
int main(int argc, char **argv)
{
    ...
    const MevaCoin::Currency currency
        = MevaCoin::CurrencyBuilder(logManager).currency();

	System::Dispatcher dispatcher;

    /* Our connection to daemon */
	MevaCoin::INode* node = new MevaCoin::NodeRpcProxy(config.host, config.port, config.path, config.ssl);

    // Set ssl options
    if (!config.daemonCert.empty()) node->setRootCert(config.daemonCert);
    if (config.disableVerify) node->disableVerify();

	std::unique_ptr<MevaCoin::INode> nodeGuard(node);
	...
	*/
    ...
    std::stringstream feemsg;
      ...
      (nodeFee == 0 ? "0.25% of transaction amount, but no more than " +
        formatAmount(MevaCoin::parameters::COIN) : formatAmount(nodeFee)) <<
			std::endl << std::endl <<
	...
	MevaCoin::WalletGreen* wallet = new MevaCoin::WalletGreen(dispatcher, currency, *node, logManager);
	std::unique_ptr<MevaCoin::WalletGreen> walletGuard(wallet);
