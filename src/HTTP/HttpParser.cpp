...
void throwIfNotGood(std::istream& stream) {
  if (!stream.good()) {
    if (stream.eof()) {
      throw std::system_error(make_error_code(MevaCoin::error::HttpParserErrorCodes::END_OF_STREAM));
    } else {
      throw std::system_error(make_error_code(MevaCoin::error::HttpParserErrorCodes::STREAM_NOT_GOOD));
    }
  }
}
...
HttpResponse::HTTP_STATUS HttpParser::parseResponseStatusFromString(const std::string& status) {
  if (status == "200 OK" || status == "200 Ok") return MevaCoin::HttpResponse::STATUS_200;
  else if (status.substr(0, 4) == "401 ") return MevaCoin::HttpResponse::STATUS_401;
  else if (status == "404 Not Found") return MevaCoin::HttpResponse::STATUS_404;
  else if (status == "500 Internal Server Error") return MevaCoin::HttpResponse::STATUS_500;
  else throw std::system_error(make_error_code(MevaCoin::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL),
      "Unknown HTTP status code is given");

  return MevaCoin::HttpResponse::STATUS_200; //unaccessible
}
...
void HttpParser::readWord(std::istream& stream, std::string& word) {
  ...
  if (c == '\r') {
    stream.get(c);
    if (c != '\n') {
      throw std::system_error(make_error_code(MevaCoin::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL));
    }
  }
}
...
bool HttpParser::readHeader(std::istream& stream, std::string& name, std::string& value) {
  ...
  while (stream.good() && c != '\r') {
    if (c == ':') {
      ...
      if (name.empty()) {
        throw std::system_error(make_error_code(MevaCoin::error::HttpParserErrorCodes::EMPTY_HEADER));
      }
