...
InProcessNode::InProcessNode(MevaCoin::ICore& core, MevaCoin::IMevaCoinProtocolQuery& protocol) :
...
void InProcessNode::init(const Callback& callback) {
  ...
  if (state != NOT_INITIALIZED) {
    ec = make_error_code(MevaCoin::error::ALREADY_INITIALIZED);
  } else {
...
void InProcessNode::getNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<MevaCoin::block_complete_entry>& newBlocks,
  uint32_t& startHeight, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
    callback(make_error_code(MevaCoin::error::NOT_INITIALIZED));
    return;
  }
...
void InProcessNode::getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<MevaCoin::block_complete_entry>& newBlocks,
  uint32_t& startHeight, const Callback& callback)
{
...
std::error_code InProcessNode::doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<MevaCoin::block_complete_entry>& newBlocks, uint32_t& startHeight) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
  ...
  try {
    ...
    if (knownBlockIds.empty()) {
      return make_error_code(MevaCoin::error::REQUEST_ERROR);
    }
    ...
    std::vector<Crypto::Hash> supplement = core.findBlockchainSupplement(knownBlockIds, MevaCoin::COMMAND_RPC_GET_BLOCKS_FAST_MAX_COUNT, totalBlockCount, startHeight);
    ...
    for (const auto& blockId : supplement) {
      ...
      MevaCoin::block_complete_entry be;
    ...
    return make_error_code(MevaCoin::error::INTERNAL_NODE_ERROR);
  }
...
//it's always protected with mutex
std::error_code InProcessNode::doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount, std::vector<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(MevaCoin::error::NOT_INITIALIZED);
  ...
  try {
    MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response res;
    MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request req;
    req.amounts = amounts;
...
void InProcessNode::relayTransaction(const MevaCoin::Transaction& transaction, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
void InProcessNode::relayTransactionAsync(const MevaCoin::Transaction& transaction, const Callback& callback) {
  std::error_code ec = doRelayTransaction(transaction);
  callback(ec);
}

//it's always protected with mutex
std::error_code InProcessNode::doRelayTransaction(const MevaCoin::Transaction& transaction) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(MevaCoin::error::NOT_INITIALIZED);
  ...
  try {
    MevaCoin::BinaryArray transactionBinaryArray = toBinaryArray(transaction);
    MevaCoin::tx_verification_context tvc = boost::value_initialized<MevaCoin::tx_verification_context>();
    ...
    MevaCoin::NOTIFY_NEW_TRANSACTIONS::request r;
...
uint64_t InProcessNode::getNextReward() const {
  ...
  if (!core.getBackwardBlocksSizes(core.getCurrentBlockchainHeight() - 1, blocksSizes, parameters::MEVACOIN_REWARD_BLOCKS_WINDOW)) {
    return false;
  }
...
void InProcessNode::resetLastLocalBlockHeaderInfo() {
  ...
  lastLocalBlockHeaderInfo.hash = MevaCoin::NULL_HASH;
  lastLocalBlockHeaderInfo.prevHash = MevaCoin::NULL_HASH;
...
std::error_code InProcessNode::doQueryBlocksLite(std::vector<Crypto::Hash>&& knownBlockIds, uint64_t timestamp, std::vector<BlockShortEntry>& newBlocks, uint32_t& startHeight) {
  ...
  std::vector<MevaCoin::BlockShortInfo> entries;
...
void InProcessNode::getTransaction(const Crypto::Hash& transactionHash, MevaCoin::Transaction& transaction, const Callback& callback) {
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
    callback(make_error_code(MevaCoin::error::NOT_INITIALIZED));
...
void InProcessNode::getTransactionAsync(const Crypto::Hash& transactionHash, MevaCoin::Transaction& transaction, const Callback& callback) {
  std::error_code ec = core.executeLocked(
    std::bind(
      static_cast<
      std::error_code(InProcessNode::*)(
...
std::error_code InProcessNode::doGetTransaction(const Crypto::Hash& transactionHash, MevaCoin::Transaction& transaction) {
  try {
...
std::error_code InProcessNode::doGetConnections(std::vector<p2pConnection>& connections) {
  std::vector<MevaCoinConnectionContext> peers;
