// Copyright (c) 2012-2016, The MevaCoin developers, The Bytecoin developers, The Monero developers
...
bool serialize(std::vector<std::pair<Blockchain::TransactionIndex, uint16_t>>& value, Common::StringView name, MevaCoin::ISerializer& s) {
...
bool Blockchain::checkTransactionInputs(const MevaCoin::Transaction& tx, BlockInfo& maxUsedBlock) {
  return checkTransactionInputs(tx, maxUsedBlock.height, maxUsedBlock.id);
}

bool Blockchain::checkTransactionInputs(const MevaCoin::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) {
...
bool Blockchain::haveSpentKeyImages(const MevaCoin::Transaction& tx) {
...
bool Blockchain::switch_to_alternative_blockchain(const std::list<Crypto::Hash>& alt_chain, bool discard_disconnected_chain) {
  ...
  if (alt_chain.size() >= MevaCoin::parameters::POISSON_CHECK_TRIGGER)
  {
    ...
    uint64_t block_ftl = MevaCoin::parameters::MEVACOIN_BLOCK_FUTURE_TIME_LIMIT_V1;
    ...
    for (; i <= MevaCoin::parameters::POISSON_CHECK_DEPTH; i++)
    {
      ...
      double lam = double(high_timestamp - low_timestamp) / double(MevaCoin::parameters::DIFFICULTY_TARGET);
      if (calc_poisson_ln(lam, alt_chain_size + i) < MevaCoin::parameters::POISSON_LOG_P_REJECT)
      {
...
bool Blockchain::prevalidate_miner_transaction(const Block& b, uint32_t height) {
  ...
  if (height > MevaCoin::parameters::UPGRADE_HEIGHT_V4_2 && extraSize > MevaCoin::parameters::MAX_EXTRA_SIZE) {
    logger(ERROR, BRIGHT_RED)
      << "The miner transaction extra is too large in block "
      << get_block_hash(b) << ". Allowed: "
      << MevaCoin::parameters::MAX_EXTRA_SIZE
      << ", actual: " << extraSize << ".";
    return false;
  }
...
bool Blockchain::checkProofOfWork(Crypto::cn_context& context, const Block& block, difficulty_type currentDiffic, Crypto::Hash& proofOfWork, const std::list<Crypto::Hash>& alt_chain, bool no_blobs) {
  if (block.majorVersion < MevaCoin::BLOCK_MAJOR_VERSION_5)
...
bool Blockchain::handle_alternative_block(const Block& b, const Crypto::Hash& id, block_verification_context& bvc, bool sendNewAlternativeBlockMessage) {
  ...
  if (it_prev != m_alternative_chains.end() || mainPrev) {
    ...
    if (bei.bl.majorVersion >= MevaCoin::BLOCK_MAJOR_VERSION_5) {
...
bool Blockchain::handleGetObjects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with MevaCoinProtocolHandler.
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, const Crypto::Hash& blockHash, block_verification_context& bvc) {
  ...
  // Disable merged mining
  if (blockData.majorVersion >= MevaCoin::BLOCK_MAJOR_VERSION_5) {
    TransactionExtraMergeMiningTag mmTag;
