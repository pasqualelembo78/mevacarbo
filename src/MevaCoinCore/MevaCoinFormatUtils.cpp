...
bool getTransactionProof(const Crypto::Hash& transactionHash, const MevaCoin::AccountPublicAddress& destinationAddress, const Crypto::SecretKey& transactionKey, std::string& transactionProof, Logging::ILogger& log) {
  ...
  transactionProof = Tools::Base58::encode_addr(MevaCoin::parameters::MEVACOIN_TX_PROOF_BASE58_PREFIX, std::string((const char *)&rA, sizeof(Crypto::PublicKey)) + std::string((const char *)&sig, sizeof(Crypto::Signature)));
...
bool getReserveProof(const std::vector<TransactionOutputInformation>& selectedTransfers, const MevaCoin::AccountKeys& accountKeys, const uint64_t& amount, const std::string& message, std::string& reserveProof, Logging::ILogger& log) {
  ...
  prefix_data.append((const char*)&accountKeys.address, sizeof(MevaCoin::AccountPublicAddress));

  std::vector<Crypto::KeyImage> kimages;
  MevaCoin::KeyPair ephemeral;
  ...
  for (size_t i = 0; i < selectedTransfers.size(); ++i) {
    ...
    Crypto::KeyImage ki;
    const bool r = MevaCoin::generate_key_image_helper(accountKeys, td.transactionPublicKey, td.outputInTransaction, ephemeral, ki);
    if (!r) {
  ...
  reserveProof = Tools::Base58::encode_addr(MevaCoin::parameters::MEVACOIN_RESERVE_PROOF_BASE58_PREFIX, ret);
...
std::string signMessage(const std::string &data, const MevaCoin::AccountKeys &keys) {
  ...
  return Tools::Base58::encode_addr(MevaCoin::parameters::MEVACOIN_KEYS_SIGNATURE_BASE58_PREFIX, std::string((const char *)&signature, sizeof(signature)));
}
...
bool verifyMessage(const std::string &data, const MevaCoin::AccountPublicAddress &address, const std::string &signature, Logging::ILogger& log) {
  ...
  std::string decoded;
  uint64_t prefix;
  if (!Tools::Base58::decode_addr(signature, prefix, decoded) || prefix != MevaCoin::parameters::MEVACOIN_KEYS_SIGNATURE_BASE58_PREFIX) {
