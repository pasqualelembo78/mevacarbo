...
namespace MevaCoin {
  ...
  std::list<MevaCoin::tx_memory_pool::TransactionDetails> tx_memory_pool::getMemoryPool() const {
	  std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
...
#define CURRENT_MEMPOOL_ARCHIVE_VER 1
  ...
  void serialize(MevaCoin::tx_memory_pool::TransactionDetails& td, ISerializer& s) {
    s(td.id, "id");
