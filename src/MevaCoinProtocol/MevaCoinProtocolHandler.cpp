...
#include "MevaCoinProtocolHandler.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const MevaCoinConnectionContext& context) {
...
MevaCoinProtocolHandler::MevaCoinProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
  ...
  m_observedHeight(0),
  m_peersCount(0),
  m_dandelionStemSelectInterval(MevaCoin::parameters::DANDELION_EPOCH),
  m_dandelionStemFluffInterval(MevaCoin::parameters::DANDELION_STEM_EMBARGO),
  logger(log, "protocol"),
  m_stemPool() {
...
size_t MevaCoinProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void MevaCoinProtocolHandler::printDandelions() const {
...
void MevaCoinProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void MevaCoinProtocolHandler::onConnectionOpened(MevaCoinConnectionContext& context) {
}

void MevaCoinProtocolHandler::onConnectionClosed(MevaCoinConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IMevaCoinProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != MevaCoinConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IMevaCoinProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void MevaCoinProtocolHandler::stop() {
  m_stop = true;
}

bool MevaCoinProtocolHandler::start_sync(MevaCoinConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == MevaCoinConnectionContext::state_synchronizing) {
...
bool MevaCoinProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void MevaCoinProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const MevaCoinConnectionContext& cntxt, PeerIdType peer_id) {
...
bool MevaCoinProtocolHandler::getConnections(std::vector<MevaCoinConnectionContext>& connections) const {
  m_p2p->for_each_connection([&](const MevaCoinConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t MevaCoinProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool MevaCoinProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, MevaCoinConnectionContext& context, bool is_initial) {
  if (context.m_state == MevaCoinConnectionContext::state_befor_handshake && !is_initial)
    return true;
  ...
  if (context.m_state == MevaCoinConnectionContext::state_synchronizing) {
    ...
    if (is_initial) {
      ...
      context.m_state = MevaCoinConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = MevaCoinConnectionContext::state_normal;
    }
  } else {
    ...
    if (diff < 0 && std::abs(diff) > (int64_t)MevaCoin::parameters::MEVACOIN_MINED_MONEY_UNLOCK_WINDOW && m_core.isInCheckpointZone(hshd.current_height)) {
    ...
    context.m_state = MevaCoinConnectionContext::state_sync_required;
  }
...
bool MevaCoinProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, MevaCoinConnectionContext& ctx, Handler handler) {
...
int MevaCoinProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, MevaCoinConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &MevaCoinProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &MevaCoinProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &MevaCoinProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &MevaCoinProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &MevaCoinProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &MevaCoinProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &MevaCoinProtocolHandler::handle_request_tx_pool)
    HANDLE_NOTIFY(NOTIFY_NEW_LITE_BLOCK, &MevaCoinProtocolHandler::handle_notify_new_lite_block)
    HANDLE_NOTIFY(NOTIFY_MISSING_TXS, &MevaCoinProtocolHandler::handle_notify_missing_txs)
...
int MevaCoinProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, MevaCoinConnectionContext& context) {
  ...
  if (context.m_state != MevaCoinConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    MevaCoin::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = MevaCoinConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int MevaCoinProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, MevaCoinConnectionContext& context) {
  ...
  if (arg.txs.size()) {
    ...
    if (arg.stem && !m_dandelion_stem.empty()) {
      ...
      if (coin_flip < MevaCoin::parameters::DANDELION_STEM_TX_PROPAGATION_PROBABILITY) { // Stem propagation
        for (const auto& dandelion_peer : m_dandelion_stem) {
          if (dandelion_peer.m_state == MevaCoinConnectionContext::state_normal || dandelion_peer.m_state == MevaCoinConnectionContext::state_synchronizing) {
...
int MevaCoinProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, MevaCoinConnectionContext& context) {
...
int MevaCoinProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, MevaCoinConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(blockHash)) {
        context.m_state = MevaCoinConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == MevaCoinConnectionContext::state_synchronizing) {
...
int MevaCoinProtocolHandler::processObjects(MevaCoinConnectionContext& context, const std::vector<parsed_block_entry>& blocks) {
...
bool MevaCoinProtocolHandler::select_dandelion_stem() {
  m_init_select_dandelion_called = true;

  m_dandelion_stem.clear();

  std::vector<MevaCoinConnectionContext> alive_peers;
  m_p2p->for_each_connection([&](const MevaCoinConnectionContext& ctx, PeerIdType peer_id) {
    if ((ctx.m_state == MevaCoinConnectionContext::state_normal || 
  ...
  if (alive_peers.size() > 0) {
    ...
    while (m_dandelion_stem.size() < std::min<size_t>(MevaCoin::parameters::DANDELION_STEMS, alive_peers.size()) && !peersGenerator.empty()) {
...
bool MevaCoinProtocolHandler::fluffStemPool() {
...
bool MevaCoinProtocolHandler::on_idle() {
  try {
    m_core.on_idle();
    // We don't have peers yet to select dandelion stems
    if (m_init_select_dandelion_called) {
      m_dandelionStemSelectInterval.call(std::bind(&MevaCoinProtocolHandler::select_dandelion_stem, this));
    }
    m_dandelionStemFluffInterval.call(std::bind(&MevaCoinProtocolHandler::fluffStemPool, this));
...
int MevaCoinProtocolHandler::doPushLiteBlock(NOTIFY_NEW_LITE_BLOCK::request arg, MevaCoinConnectionContext &context,
...
int MevaCoinProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, MevaCoinConnectionContext& context) {
...
bool MevaCoinProtocolHandler::request_missing_objects(MevaCoinConnectionContext& context, bool check_having_blocks) {
...
bool MevaCoinProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IMevaCoinProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int MevaCoinProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, MevaCoinConnectionContext& context) {
...
int MevaCoinProtocolHandler::handle_request_tx_pool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     MevaCoinConnectionContext& context) {
...
int MevaCoinProtocolHandler::handle_notify_new_lite_block(int command, NOTIFY_NEW_LITE_BLOCK::request &arg,
                                                           MevaCoinConnectionContext &context) {
...
int MevaCoinProtocolHandler::handle_notify_missing_txs(int command,  NOTIFY_MISSING_TXS::request &arg,
                                                        MevaCoinConnectionContext &context) {
...
void MevaCoinProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  ...
  m_p2p->for_each_connection([this, &liteBlockConnections, &normalBlockConnections](
    const MevaCoinConnectionContext &ctx, uint64_t peerId) {
...
void MevaCoinProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
...
void MevaCoinProtocolHandler::requestMissingPoolTransactions(const MevaCoinConnectionContext& context) {
  if (context.version < MevaCoin::P2P_VERSION_1) {
    return;
  }
...
void MevaCoinProtocolHandler::updateObservedHeight(uint32_t peerHeight, const MevaCoinConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void MevaCoinProtocolHandler::recalculateMaxObservedHeight(const MevaCoinConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const MevaCoinConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t MevaCoinProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool MevaCoinProtocolHandler::addObserver(IMevaCoinProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool MevaCoinProtocolHandler::removeObserver(IMevaCoinProtocolObserver* observer) {
