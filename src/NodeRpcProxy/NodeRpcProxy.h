...
class NodeRpcProxy : public MevaCoin::INode {
public:
  ...
  virtual bool addObserver(MevaCoin::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(MevaCoin::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<MevaCoin::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<MevaCoin::INodeObserver> m_observerManager;
  Tools::ObserverManager<MevaCoin::INodeRpcProxyObserver> m_rpcProxyObserverManager;
