...
const MevaCoin::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  MevaCoin::Core core(currency, NULL, logger, *dispatcher, false);

  MevaCoin::MevaCoinProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  MevaCoin::NodeServer p2pNode(*dispatcher, protocol, logger);

  MevaCoin::Checkpoints checkpoints(logger);
  ...
  core.set_mevacoin_protocol(&protocol);
  ...
  MevaCoin::MinerConfig emptyMiner;
  core.init(config.coreConfig, emptyMiner, true);

  std::promise<std::error_code> initPromise;
  auto initFuture = initPromise.get_future();

  std::unique_ptr<MevaCoin::INode> node(new MevaCoin::InProcessNode(core, protocol));
...
void PaymentGateService::runWalletServiceOr(const MevaCoin::Currency& currency, MevaCoin::INode& node) {
...
void PaymentGateService::runWalletService(const MevaCoin::Currency& currency, MevaCoin::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<MevaCoin::WalletGreen> wallet(new MevaCoin::WalletGreen(*dispatcher, currency, node, logger));
