...
#include "MevaCoinCore/TransactionUtils.h"
...
RpcServer::RpcServer(
  ...
  Logging::ILogger& log,
  MevaCoin::Core& core,
  NodeServer& p2p, IMevaCoinProtocolQuery& protocolQuery
) :
  m_config(config),
...
bool RpcServer::checkIncomingTransactionForFee(const BinaryArray& tx_blob) {
  ...
  MevaCoin::TransactionPrefix transaction = *static_cast<const TransactionPrefix*>(&tx);

  std::vector<uint32_t> out;
  uint64_t amount;

  MevaCoin::findOutputsToAccount(transaction, m_fee_acc, m_view_key, out, amount);
...
bool RpcServer::on_get_pool_changes(const COMMAND_RPC_GET_POOL_CHANGES::request& req, COMMAND_RPC_GET_POOL_CHANGES::response& rsp) {
  ...
  std::vector<MevaCoin::Transaction> addedTransactions;
...
bool RpcServer::on_get_explorer(const COMMAND_EXPLORER::request& req, COMMAND_EXPLORER::response& res) {
  ...
  if (req_height == top_block_index) {
    ...
    if (!pool.empty()) {
      ...
      body += "</thead>\n";
      body += "<tbody>\n";
      for (const MevaCoin::tx_memory_pool::TransactionDetails& txd : pool) {
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  MevaCoin::BinaryArray blob_reserve;
  ...
  Crypto::PublicKey tx_pub_key = MevaCoin::getTransactionPublicKeyFromExtra(b.baseTransaction.extra);
...
bool RpcServer::on_check_transaction_proof(const COMMAND_RPC_CHECK_TRANSACTION_PROOF::request& req, COMMAND_RPC_CHECK_TRANSACTION_PROOF::response& res) {
  ...
  uint64_t prefix;
  if (!Tools::Base58::decode_addr(req.signature, prefix, decoded_data) || prefix != MevaCoin::parameters::MEVACOIN_TX_PROOF_BASE58_PREFIX) {
...
bool RpcServer::on_check_reserve_proof(const COMMAND_RPC_CHECK_RESERVE_PROOF::request& req, COMMAND_RPC_CHECK_RESERVE_PROOF::response& res) {
  ...
  std::string decoded_data;
  uint64_t prefix;
  if (!Tools::Base58::decode_addr(req.signature, prefix, decoded_data) || prefix != MevaCoin::parameters::MEVACOIN_RESERVE_PROOF_BASE58_PREFIX) {
  ...
  prefix_data.append((const char*)&address, sizeof(MevaCoin::AccountPublicAddress));
  ...
  for (size_t i = 0; i < proofs.size(); ++i) {
    ...
    MevaCoin::TransactionPrefix tx = *static_cast<const TransactionPrefix*>(&transactions[i]);
...
bool RpcServer::on_verify_message(const COMMAND_RPC_VERIFY_MESSAGE::request& req, COMMAND_RPC_VERIFY_MESSAGE::response& res) {
  ...
  //res.sig_valid = MevaCoin::verifyMessage(req.message, acc, req.signature, logger.getLogger());

  std::string decoded;
  Crypto::Signature s;
  uint64_t prefix;
  if (!Tools::Base58::decode_addr(req.signature, prefix, decoded) || prefix != MevaCoin::parameters::MEVACOIN_KEYS_SIGNATURE_BASE58_PREFIX) {
