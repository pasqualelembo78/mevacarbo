...
namespace MevaCoin {
  ...
  void RpcServerConfig::init(const boost::program_options::variables_map& vm)  {
    ...
    if ((nodeFeeAddress.empty() && !nodeFeeAmountStr.empty()) ||
      ...
      if (!MevaCoin::parseAccountAddressString(prefix, acc, nodeFeeAddress)) {
      ...
      if (nodeFeeAmount > MevaCoin::parameters::COIN) {
        throw std::runtime_error("Maximum allowed fee is " + Common::Format::formatAmount(MevaCoin::parameters::COIN));
      }
    }
