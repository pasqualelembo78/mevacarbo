...
template<typename K, typename V, typename Hash>
bool serialize(flat_hash_map<K, V, Hash>& value, Common::StringView name, MevaCoin::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [](size_t size) {});
}

template<typename K, typename V, typename Hash>
bool serialize(parallel_flat_hash_map<K, V, Hash>& value, Common::StringView name, MevaCoin::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [](size_t size) {});
}
