...
// Copyright (c) 2016-2021, The Mevacoin Developers
//
...
public:
  explicit TransactionReaderListFormatter(const std::vector<std::unique_ptr<MevaCoin::ITransactionReader>>& transactionList) :
...
private:
  const std::vector<std::unique_ptr<MevaCoin::ITransactionReader>>& m_transactionList;
};
...
std::ostream& operator<<(std::ostream& os, const MevaCoin::IBlockchainConsumer* consumer) {
