...
std::vector<Crypto::Hash> getBlockHashes(const MevaCoin::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const MevaCoin::Currency& currency, INode& node, Logging::ILogger& logger, const SecretKey& viewSecret) :
...
std::error_code TransfersConsumer::onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>>& addedTransactions, const std::vector<Hash>& deletedTransactions) {
  ...
  std::error_code processingError;
  for (auto& mevacoinTransaction : addedTransactions) {
    m_poolTxs.emplace(mevacoinTransaction->getTransactionHash());
    processingError = processTransaction(unconfirmedBlockInfo, *mevacoinTransaction.get());
    if (processingError) {
...
std::error_code createTransfers(
  ...
  for (auto idx : outputs) {
    ...
    if (outType == TransactionTypes::OutputType::Key) {
      ...
      MevaCoin::generate_key_image_helper(
        account,
        txPubKey,
        idx,
        in_ephemeral,
