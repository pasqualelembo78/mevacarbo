...
void WalletGreen::prepareTransaction(std::vector<WalletOuts>&& wallets,
  ...
  typedef MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount outs_for_amount;
...
void WalletGreen::checkIfEnoughMixins(std::vector<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) const {
  assert(mixIn != 0);

  auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
    [mixIn](const MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount& ofa) { return ofa.outs.size() < mixIn; });
...
void WalletGreen::sendTransaction(const MevaCoin::Transaction& mevaCoinTransaction) {
  std::error_code ec;
  ...
  try {
    ...
    m_node.relayTransaction(mevaCoinTransaction, [&ec, &relayTransactionCompleted, this](std::error_code error) {
      auto detachedPromise = std::move(relayTransactionCompleted);
    detachedPromise.set_value(ec);
    });
    ec = relayTransactionWaitFuture.get();
  ...
  if (ec) {
    m_logger(ERROR, BRIGHT_RED) << "Failed to relay transaction: " << ec << ", " << ec.message() <<
      ". Transaction hash " << getObjectHash(mevaCoinTransaction);
    throw std::system_error(ec);
  }
}
...
size_t WalletGreen::validateSaveAndSendTransaction(const ITransactionReader& transaction, const std::vector<WalletTransfer>& destinations, bool isFusion, bool send) {
  ...
  MevaCoin::Transaction mevaCoinTransaction;
  if (!fromBinaryArray(mevaCoinTransaction, transactionData)) {
  ...
  if (send) {
    sendTransaction(mevaCoinTransaction);
    ...
    m_uncommitedTransactions.emplace(transactionId, std::move(mevaCoinTransaction));
...
void WalletGreen::requestMixinOuts(
  ...
  uint64_t mixIn,
  std::vector<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult) {
...
void WalletGreen::prepareInputs(
  ...
  typedef MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry out_entry;
...
Crypto::SecretKey WalletGreen::getTransactionDeterministicSecretKey(Crypto::Hash& transactionHash) const {
  ...
  Crypto::SecretKey txKey = MevaCoin::NULL_SECRET_KEY;
  ...
  if (ec) {
    ...
    return MevaCoin::NULL_SECRET_KEY;
  }
  ...
  return ok ? deterministicTxKeys.secretKey : MevaCoin::NULL_SECRET_KEY;
...
Crypto::SecretKey WalletGreen::getTransactionSecretKey(size_t transactionIndex) const {
  ...
  Crypto::SecretKey txKey = m_transactions.get<RandomAccessIndex>()[transactionIndex].secretKey.get_value_or(MevaCoin::NULL_SECRET_KEY);
  if (txKey == MevaCoin::NULL_SECRET_KEY) {
...
Crypto::SecretKey WalletGreen::getTransactionSecretKey(Crypto::Hash& transactionHash) const {
  ...
  Crypto::SecretKey txKey = txInfo.transaction.secretKey.get_value_or(MevaCoin::NULL_SECRET_KEY);
...
bool WalletGreen::getTransactionProof(const Crypto::Hash& transactionHash, const MevaCoin::AccountPublicAddress& destinationAddress, const Crypto::SecretKey& txKey, std::string& transactionProof) {
  return MevaCoin::getTransactionProof(transactionHash, destinationAddress, txKey, transactionProof, m_logger.getLogger());
}
...
std::string WalletGreen::getReserveProof(const uint64_t &reserve, const std::string& address, const std::string &message) {
  ...
  bool r = MevaCoin::getReserveProof(selectedTransfers, keys, reserve, message, reserveProof, m_logger.getLogger());
  if (!r) {
...
std::string WalletGreen::signMessage(const std::string &message, const std::string& address) {
  ...
  return MevaCoin::signMessage(message, keys);
}
...
bool WalletGreen::verifyMessage(const std::string &message, const std::string& address, const std::string &signature) {
  ...
  try {
    ...
    return MevaCoin::verifyMessage(message, pubAddr, signature, m_logger.getLogger());
  }
  catch (const std::exception& e) {
...
void WalletGreen::transactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  bool updated = false;
  m_transactions.get<TransactionIndex>().modify(it, [this, &transactionHash, &updated](MevaCoin::WalletTransaction& tx) {
