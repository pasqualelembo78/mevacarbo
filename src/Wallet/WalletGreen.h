...
public:
  WalletGreen(System::Dispatcher& dispatcher, const Currency& currency, INode& node, Logging::ILogger& logger, uint32_t transactionSoftLockTime = MevaCoin::parameters::MEVACOIN_TX_SPENDABLE_AGE);
  virtual ~WalletGreen();
  ...
  virtual bool getTransactionProof(const Crypto::Hash& transactionHash, const MevaCoin::AccountPublicAddress& destinationAddress, const Crypto::SecretKey& txKey, std::string& transactionProof) override;
...
protected:
  ...
  MevaCoin::BlockDetails getBlock(const uint32_t blockHeight);
  ...
  struct ReceiverAmounts {
    MevaCoin::AccountPublicAddress receiver;
...
#pragma pack(pop)
  ...
  void updateBalance(MevaCoin::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(MevaCoin::ITransfersContainer* container) const;

  MevaCoin::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const MevaCoin::AccountPublicAddress& changeDestinationAddress,
    PreparedTransaction& preparedTransaction,
    Crypto::SecretKey& txSecretKey);

  size_t doTransfer(const TransactionParameters& transactionParameters, Crypto::SecretKey& txSecretKey);

  void checkIfEnoughMixins(std::vector<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) const;
  std::vector<WalletTransfer> convertOrdersToTransfers(const std::vector<WalletOrder>& orders) const;
  uint64_t countNeededMoney(const std::vector<MevaCoin::WalletTransfer>& destinations, uint64_t fee) const;
  MevaCoin::AccountPublicAddress parseAccountAddressString(const std::string& addressString) const;
  ...
  std::unique_ptr<MevaCoin::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp, Crypto::SecretKey& txSecretKey);

  void sendTransaction(const MevaCoin::Transaction& mevaCoinTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const MevaCoin::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, MevaCoin::ITransfersContainer* container);
  ...
  MevaCoin::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
  ...
  friend std::ostream& operator<<(std::ostream& os, MevaCoin::WalletGreen::WalletState state);
  friend std::ostream& operator<<(std::ostream& os, MevaCoin::WalletGreen::WalletTrackingMode mode);
