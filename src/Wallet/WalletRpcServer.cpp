...
{
  using namespace MevaCoin::JsonRpc;
  ...
  wallet_rpc::COMMAND_RPC_TRANSFER::response& res)
{
  ...
  if (!req.payment_id.empty())
  {
    ...
    if (!MevaCoin::parsePaymentId(payment_id_str, payment_id))
    {
    ...
    MevaCoin::setPaymentIdToTransactionExtraNonce(extra_nonce, payment_id);
    if (!MevaCoin::addExtraNonceToTransactionExtra(extra, extra_nonce))
    {
      throw JsonRpc::JsonRpcError(WALLET_RPC_ERROR_CODE_WRONG_PAYMENT_ID,
  ...
  try
  {
    ...
    MevaCoin::TransactionId tx = m_wallet.sendTransaction(transfers, req.fee == 0 ? m_currency.minimumFee() : req.fee, extraString, req.mixin, req.unlock_time);
  ...
  wallet_rpc::COMMAND_RPC_GET_PAYMENTS::response& res)
{
  ...
  MevaCoin::BinaryArray payment_id_blob;
  ...
  wallet_rpc::COMMAND_RPC_QUERY_KEY::response& res)
{
  ...
  if (0 == req.key_type.compare("paperwallet")) {
    ...
    res.key = Tools::Base58::encode_addr(parameters::MEVACOIN_PUBLIC_ADDRESS_BASE58_PREFIX,
...
bool wallet_rpc_server::on_get_tx_proof(const wallet_rpc::COMMAND_RPC_GET_TX_PROOF::request& req,
  ...
  MevaCoin::AccountPublicAddress dest_address;
...
bool wallet_rpc_server::on_send_fusion(const wallet_rpc::COMMAND_RPC_SEND_FUSION::request& req, wallet_rpc::COMMAND_RPC_SEND_FUSION::response& res)
{
  ...
  try {
    ...
    MevaCoin::TransactionId tx = m_wallet.sendFusionTransaction(fusionInputs, 0, extraString, req.mixin, req.unlock_time);
    if (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID)
