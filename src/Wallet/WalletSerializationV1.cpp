...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const MevaCoin::WalletTransaction& wallet) {
    state = wallet.state;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const MevaCoin::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, MevaCoin::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, MevaCoin::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, MevaCoin::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, MevaCoin::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, MevaCoin::ISerializer& serializer) {
  ...
  MevaCoin::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, MevaCoin::ISerializer& serializer) {
...
std::string decrypt(const std::string& cipher, MevaCoin::WalletSerializerV1::CryptoContext& cryptoContext) {
  std::string plain;
...
template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, MevaCoin::WalletSerializerV1::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
MevaCoin::WalletTransaction convert(const MevaCoin::WalletLegacyTransaction& tx) {
  MevaCoin::WalletTransaction mtx;

  mtx.state = MevaCoin::WalletTransactionState::SUCCEEDED;
...
MevaCoin::WalletTransfer convert(const MevaCoin::WalletLegacyTransfer& tr) {
  MevaCoin::WalletTransfer mtr;
...
void WalletSerializerV1::loadWalletV1(Common::IInputStream& source, const Crypto::chacha8_key& key) {
  ...
  MevaCoin::BinaryInputStreamSerializer encrypted(source);
...
void WalletSerializerV1::loadWalletV1Keys(MevaCoin::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializerV1::loadWalletV1Details(MevaCoin::BinaryInputStreamSerializer& serializer) {
