...
std::error_code walletSaveWrapper(MevaCoin::IWalletLegacy& wallet, std::ofstream& file, bool saveDetailes, bool saveCache) {
  MevaCoin::WalletHelper::SaveWalletResultObserver o;
...
void WalletHelper::SendCompleteResultObserver::sendTransactionCompleted(MevaCoin::TransactionId transactionId, std::error_code result) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_finishedTransactions[transactionId] = result;
  m_condition.notify_one();
}

std::error_code WalletHelper::SendCompleteResultObserver::wait(MevaCoin::TransactionId transactionId) {
...
WalletHelper::IWalletRemoveObserverGuard::IWalletRemoveObserverGuard(MevaCoin::IWalletLegacy& wallet, MevaCoin::IWalletLegacyObserver& observer) :
  m_wallet(wallet),
  m_observer(observer),
  m_removed(false) {
...
bool WalletHelper::storeWallet(MevaCoin::IWalletLegacy& wallet, const std::string& walletFilename) {
