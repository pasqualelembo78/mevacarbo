...
void serialize(UnconfirmedTransferDetails& utd, MevaCoin::ISerializer& serializer) {
  ...
  if (MevaCoin::WALLET_LEGACY_SERIALIZATION_VERSION >= 2)
    serializer(utd.secretKey, "secret_key");
}

void serialize(WalletLegacyTransaction& txi, MevaCoin::ISerializer& serializer) {
  ...
  MevaCoin::serializeBlockHeight(serializer, txi.blockHeight, "block_height");
...
void serialize(WalletLegacyTransfer& tr, MevaCoin::ISerializer& serializer) {
