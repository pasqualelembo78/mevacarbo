...
const uint64_t UNDEF_HEIGHT = static_cast<uint64_t>(MevaCoin::UpgradeDetectorBase::UNDEF_HEIGHT);
...
public:
  CheckBlockPurged(size_t invalidBlockIdx, uint8_t blockMajorVersion) :
    ...
    assert(blockMajorVersion == MevaCoin::BLOCK_MAJOR_VERSION_1 || blockMajorVersion == MevaCoin::BLOCK_MAJOR_VERSION_2);

    MevaCoin::CurrencyBuilder currencyBuilder(m_logger);
    currencyBuilder.upgradeHeightV2(blockMajorVersion == MevaCoin::BLOCK_MAJOR_VERSION_1 ? UNDEF_HEIGHT : UINT64_C(0));
  ...
  bool check_block_purged(MevaCoin::core& c, size_t eventIdx, const std::vector<test_event_entry>& events) {
  ...
  bool markInvalidBlock(MevaCoin::core& c, size_t eventIdx, const std::vector<test_event_entry>& events) {
...
struct CheckBlockAccepted : public test_chain_unit_base {
  ...
  bool check_block_accepted(MevaCoin::core& c, size_t /*eventIdx*/, const std::vector<test_event_entry>& /*events*/) {
...
struct gen_block_invalid_binary_format : public test_chain_unit_base
{
  ...
  bool generate(std::vector<test_event_entry>& events) const;
  bool check_block_verification_context(const MevaCoin::block_verification_context& bvc, size_t event_idx, const MevaCoin::Block& /*blk*/);
  bool check_all_blocks_purged(MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events);
  bool corrupt_blocks_boundary(MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events);
...
struct TestMaxSizeOfParentBlock : public CheckBlockAccepted {
  TestMaxSizeOfParentBlock() : CheckBlockAccepted(2, MevaCoin::BLOCK_MAJOR_VERSION_2) {
  }
...
struct TestBigParentBlock : public CheckBlockPurged {
  TestBigParentBlock() : CheckBlockPurged(1, MevaCoin::BLOCK_MAJOR_VERSION_2) {
  }
...
struct TestBlock2ExtraEmpty : public CheckBlockPurged {
  ...
  TestBlock2ExtraEmpty() : CheckBlockPurged(1, MevaCoin::BLOCK_MAJOR_VERSION_2) {}
...
struct TestBlock2ExtraWithoutMMTag : public CheckBlockPurged {
  ...
  TestBlock2ExtraWithoutMMTag() : CheckBlockPurged(1, MevaCoin::BLOCK_MAJOR_VERSION_2) {}
...
struct TestBlock2ExtraWithGarbage : public CheckBlockAccepted {
  ...
  TestBlock2ExtraWithGarbage() : CheckBlockAccepted(2, MevaCoin::BLOCK_MAJOR_VERSION_2) {}
