...
struct serialized_object
{
  ...
  serialized_object(const MevaCoin::BinaryArray& a_data)
    : data(a_data)
  {
  }

  MevaCoin::BinaryArray data;
  //BEGIN_SERIALIZE_OBJECT()
  //  FIELD(data)
  //  END_SERIALIZE()
...
typedef serialized_object<MevaCoin::Block> serialized_block;
typedef serialized_object<MevaCoin::Transaction> serialized_transaction;
...
//VARIANT_TAG(binary_archive, MevaCoin::account_base, 0xcc);
//VARIANT_TAG(binary_archive, serialized_block, 0xcd);
//VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
//VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<MevaCoin::Block, MevaCoin::Transaction, MevaCoin::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<Crypto::Hash, const MevaCoin::Transaction*> map_hash2tx_t;

class test_chain_unit_base: boost::noncopyable
{
public:
  test_chain_unit_base() :
    m_currency(MevaCoin::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  const MevaCoin::Currency& currency() const;
  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry> &events);
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, MevaCoin::Transaction& tx,
                         const MevaCoin::Block& blk_head, const MevaCoin::AccountBase& from, const MevaCoin::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
MevaCoin::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const MevaCoin::Block& blk_head,
                                            const MevaCoin::AccountBase& acc_from, const MevaCoin::AccountBase& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<MevaCoin::Block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<MevaCoin::Block>& blockchain, map_hash2tx_t& mtx, const Crypto::Hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const MevaCoin::Block& blk_head,
                                      const MevaCoin::AccountBase& from, const MevaCoin::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<MevaCoin::TransactionSourceEntry>& sources,
                                      std::vector<MevaCoin::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const MevaCoin::AccountBase& addr, const std::vector<MevaCoin::Block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const MevaCoin::tx_verification_context& tvc, bool tx_added, size_t event_index, const MevaCoin::Transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_tx_verification_context(const MevaCoin::tx_verification_context& tvc, bool tx_added, size_t /*event_index*/, const MevaCoin::Transaction& /*tx*/, t_test_class&, long)
{
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const MevaCoin::tx_verification_context& tvc, bool tx_added, size_t event_index, const MevaCoin::Transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const MevaCoin::block_verification_context& bvc, size_t event_index, const MevaCoin::Block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_block_verification_context(const MevaCoin::block_verification_context& bvc, size_t /*event_index*/, const MevaCoin::Block& /*blk*/, t_test_class&, long)
{
  // Default block verification context check
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const MevaCoin::block_verification_context& bvc, size_t event_index, const MevaCoin::Block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  push_core_event_visitor(MevaCoin::core& c, const std::vector<test_event_entry>& events, t_test_class& validator)
    : m_c(c)
    , m_events(events)
  ...
  bool operator()(const MevaCoin::Transaction& tx) const
  {
    log_event("MevaCoin::Transaction");
  ...
  bool operator()(const MevaCoin::Block& b) const
  {
    log_event("MevaCoin::Block");

    MevaCoin::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
  ...
  bool operator()(const MevaCoin::AccountBase& ab) const
  {
    log_event("MevaCoin::account_base");
    return true;
  }
  ...
  bool operator()(const serialized_block& sr_block) const
  {
    ...
    MevaCoin::Block blk;
    if (!MevaCoin::fromBinaryArray(blk, sr_block.data)) {
      blk = MevaCoin::Block();
    }
  ...
  bool operator()(const serialized_transaction& sr_tx) const
  {
    ...
    if (!MevaCoin::fromBinaryArray(tx, sr_tx.data)) {
      tx = MevaCoin::Transaction();
    }
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(MevaCoin::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  try {
    CHECK_AND_ASSERT_MES(typeid(MevaCoin::Block) == events[0].type(), false, "First event must be genesis block creation");
    cr.set_genesis_block(boost::get<MevaCoin::Block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  MevaCoin::CoreConfig::initOptions(desc);
  ...
  coreConfig.init(vm);
  MevaCoin::MinerConfig emptyMinerConfig;
  MevaCoin::mevacoin_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  MevaCoin::core c(validator.currency(), &pr, logger, false);
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  MevaCoin::Block BLK_NAME;                                                         \
  {                                                                                   \
    MevaCoin::Block blk_last = PREV_BLOCK;                                          \
...
#define MAKE_TX_LIST_START(VEC_EVENTS, SET_NAME, FROM, TO, AMOUNT, HEAD) \
    std::list<MevaCoin::Transaction> SET_NAME; \
...
const uint64_t COIN = Pow10<MevaCoin::parameters::MEVACOIN_DISPLAY_DECIMAL_POINT>::value;
