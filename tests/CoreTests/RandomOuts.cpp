...
bool GetRandomOutputs::request(MevaCoin::core& c, uint64_t amount, size_t mixin, MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(MevaCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
