...
TransactionBuilder::TransactionBuilder(const MevaCoin::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(MevaCoin::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const MevaCoin::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<MevaCoin::TransactionSourceEntry>& sources, const MevaCoin::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<MevaCoin::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const MevaCoin::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<MevaCoin::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<MevaCoin::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<MevaCoin::KeyPair>& contexts, Transaction& tx) const {
