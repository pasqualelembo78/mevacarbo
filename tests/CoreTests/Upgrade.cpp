...
namespace {
  bool makeBlocks(std::vector<test_event_entry>& events, test_generator& generator, Block& lastBlock,
                  const Block& parentBlock, const MevaCoin::AccountBase& minerAcc, size_t count,
                  uint8_t majorVersion, uint8_t minorVersion) {
    MevaCoin::Block prevBlock = parentBlock;
...
gen_upgrade::gen_upgrade() : m_invalidBlockIndex(0), m_checkBlockTemplateVersionCallCounter(0),
  ...
  currencyBuilder.upgradeHeightV3(MevaCoin::parameters::MEVACOIN_MAX_BLOCK_NUMBER);
...
bool gen_upgrade::checkBeforeUpgrade(std::vector<test_event_entry>& events, test_generator& generator,
                                     const MevaCoin::Block& parentBlock, const MevaCoin::AccountBase& minerAcc,
...
bool gen_upgrade::check_block_verification_context(const MevaCoin::block_verification_context& bvc, size_t eventIdx, const MevaCoin::Block& /*blk*/) {
...
bool gen_upgrade::markInvalidBlock(MevaCoin::core& /*c*/, size_t evIndex, const std::vector<test_event_entry>& /*events*/) {
  m_invalidBlockIndex = evIndex + 1;
  return true;
}

bool gen_upgrade::checkBlockTemplateVersionIsV1(MevaCoin::core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
...
bool gen_upgrade::checkBlockTemplateVersionIsV2(MevaCoin::core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
...
bool gen_upgrade::checkBlockTemplateVersion(MevaCoin::core& c, uint8_t expectedMajorVersion, uint8_t expectedMinorVersion) {
...
bool gen_upgrade::checkBlockRewardEqFee(MevaCoin::core& c, size_t evIndex, const std::vector<test_event_entry>& events) {
...
bool gen_upgrade::checkBlockRewardIsZero(MevaCoin::core& c, size_t evIndex, const std::vector<test_event_entry>& events) {
...
bool gen_upgrade::rememberCoinsInCirculationBeforeUpgrade(MevaCoin::core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
  m_coinsInCirculationBeforeUpgrade = c.getTotalGeneratedAmount();
  return true;
}

bool gen_upgrade::rememberCoinsInCirculationAfterUpgrade(MevaCoin::core& c, size_t /*evIndex*/, const std::vector<test_event_entry>& /*events*/) {
