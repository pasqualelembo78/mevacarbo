...
namespace {
  class WaitForCoinBaseObserver : public MevaCoin::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    MevaCoin::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, MevaCoin::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(MevaCoin::TransactionId transactionId) override {
      MevaCoin::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const MevaCoin::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, MevaCoin::Block&& blockTemplate) {
  ...
  if (blockTemplate.majorVersion >= BLOCK_MAJOR_VERSION_2) {
    ...
    MevaCoin::TransactionExtraMergeMiningTag mmTag;
    mmTag.depth = 0;
    if (!MevaCoin::get_aux_block_header_hash(blockTemplate, mmTag.merkleRoot)) {
      return false;
    }

    blockTemplate.parentBlock.baseTransaction.extra.clear();
    if (!MevaCoin::appendMergeMiningTagToExtra(blockTemplate.parentBlock.baseTransaction.extra, mmTag)) {
      return false;
    }
  }

  BinaryArray blockBlob = MevaCoin::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<MevaCoin::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<MevaCoin::IWalletLegacy> & wallet, std::unique_ptr<MevaCoin::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<MevaCoin::IWalletLegacy>(new MevaCoin::WalletLegacy(m_currency, *node, m_logger));
...
namespace {
  struct PeerCountWaiter : MevaCoin::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(MevaCoin::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, MevaCoin::INode& node, size_t expectedPoolSize,
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, MevaCoin::INode& node,
  std::vector<std::unique_ptr<MevaCoin::ITransactionReader>>& txPool) {
