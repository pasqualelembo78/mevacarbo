...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const MevaCoin::Currency& currency) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new MevaCoin::core(m_currency, NULL, log, false));
    protocol.reset(new MevaCoin::MevaCoinProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new MevaCoin::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_mevacoin_protocol(protocol.get());

    MevaCoin::NetNodeConfig p2pConfig;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_mevacoin_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, MevaCoin::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<MevaCoin::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new MevaCoin::InProcessNode(*core, *protocol));
