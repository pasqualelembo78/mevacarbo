...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, MevaCoin::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<MevaCoin::INode>& node) {
  std::unique_ptr<MevaCoin::INode> newNode(new MevaCoin::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    MevaCoin::COMMAND_RPC_GET_INFO::request req;
    MevaCoin::COMMAND_RPC_GET_INFO::response rsp;
