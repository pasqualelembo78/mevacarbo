...
public:
  ...
  IntegrationTest() : 
    currency(MevaCoin::CurrencyBuilder(log).testnet(true).currency()), 
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<MevaCoin::IWalletLegacy> wallet(new MevaCoin::WalletLegacy(m_currency, *n, log));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<MevaCoin::INode>& localNode = inodes.front();
    std::unique_ptr<MevaCoin::INode>& remoteNode = inodes.back();
