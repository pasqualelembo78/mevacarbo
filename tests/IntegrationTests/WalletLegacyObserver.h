...
public:
  ...
  MevaCoin::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    MevaCoin::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(MevaCoin::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<MevaCoin::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<MevaCoin::TransactionId, std::error_code> m_sendResults;
  std::deque<MevaCoin::TransactionId> m_externalTransactions;
};
