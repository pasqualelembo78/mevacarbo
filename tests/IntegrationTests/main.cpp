...
public:
  ...
  SimpleTest(const MevaCoin::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) : 
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public MevaCoin::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public MevaCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public MevaCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public MevaCoin::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public MevaCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    MevaCoin::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, MevaCoin::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public MevaCoin::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<MevaCoin::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public MevaCoin::IWalletLegacyObserver {
  public:
    ...
    MevaCoin::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<MevaCoin::INode> node1;
    std::unique_ptr<MevaCoin::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<MevaCoin::IWalletLegacy> wallet1;
    std::unique_ptr<MevaCoin::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public MevaCoin::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public MevaCoin::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<MevaCoin::INode> localNode;
    std::unique_ptr<MevaCoin::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<MevaCoin::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<MevaCoin::INode> hopNode;
    std::unique_ptr<MevaCoin::INode> localNode;
    std::unique_ptr<MevaCoin::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<MevaCoin::INode> node1;
    std::unique_ptr<MevaCoin::INode> inprocNode;
...
void testMultiVersion(const MevaCoin::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = MevaCoin::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
