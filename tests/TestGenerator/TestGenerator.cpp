...
uint64_t test_generator::getAlreadyGeneratedCoins(const MevaCoin::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const MevaCoin::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(MevaCoin::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
  ...
  if (blk.majorVersion >= BLOCK_MAJOR_VERSION_2) {
    ...
    mmTag.depth = 0;
    if (!MevaCoin::get_aux_block_header_hash(blk, mmTag.merkleRoot)) {
      return false;
    }

    blk.parentBlock.baseTransaction.extra.clear();
    if (!MevaCoin::appendMergeMiningTagToExtra(blk.parentBlock.baseTransaction.extra, mmTag)) {
      return false;
    }
  }
...
bool test_generator::constructBlock(MevaCoin::Block& blk, const MevaCoin::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<MevaCoin::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(MevaCoin::Block& blk, const MevaCoin::Block& blkPrev,
                                    const MevaCoin::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(MevaCoin::Block& blk, const MevaCoin::Block& prevBlock,
                                              const MevaCoin::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(MevaCoin::Block& blk, const MevaCoin::Block& blkPrev,
                                           const MevaCoin::AccountBase& minerAccount,
...
void fillNonce(MevaCoin::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const MevaCoin::Currency& currency, MevaCoin::Transaction& baseTransaction, uint8_t blockMajorVersion, uint32_t height,
  ...
  if (!currency.constructMinerTx(blockMajorVersion, height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, MevaCoin::BinaryArray(), 1)) {
    return false;
  }
