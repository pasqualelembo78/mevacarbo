...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = MevaCoin::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(MevaCoin::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = MevaCoin::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(MevaCoin::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}
