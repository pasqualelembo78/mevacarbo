...
namespace {
  using MevaCoin::BLOCK_MAJOR_VERSION_1;
  using MevaCoin::BLOCK_MAJOR_VERSION_2;
  using MevaCoin::BLOCK_MINOR_VERSION_0;
  using MevaCoin::BLOCK_MINOR_VERSION_1;
  ...
  typedef MevaCoin::BasicUpgradeDetector<BlockVector> UpgradeDetector;

  class UpgradeTest : public ::testing::Test {
  public:

    MevaCoin::Currency createCurrency(uint32_t upgradeHeight = UpgradeDetector::UNDEF_HEIGHT) {
      MevaCoin::CurrencyBuilder currencyBuilder(logger);
      ...
      currencyBuilder.upgradeHeightV3(MevaCoin::UpgradeDetectorBase::UNDEF_HEIGHT);
  ...
  TEST_F(UpgradeDetector_voting_init, handlesEmptyBlockchain) {
    MevaCoin::Currency currency = createCurrency();
    BlockVector blocks;
  ...
  TEST_F(UpgradeDetector_upgradeHeight_init, handlesEmptyBlockchain) {
    ...
    MevaCoin::Currency currency = createCurrency(upgradeHeight);
    BlockVector blocks;
    UpgradeDetector upgradeDetector(currency, blocks, BLOCK_MAJOR_VERSION_2, logger);
    ASSERT_TRUE(upgradeDetector.init());
    ASSERT_EQ(upgradeDetector.upgradeHeight(), upgradeHeight);
