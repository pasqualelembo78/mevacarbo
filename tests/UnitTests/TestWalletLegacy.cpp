...
class TrivialWalletObserver : public MevaCoin::IWalletLegacyObserver
{
public:
...
struct SaveOnInitWalletObserver: public MevaCoin::IWalletLegacyObserver {
  SaveOnInitWalletObserver(MevaCoin::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  MevaCoin::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

MevaCoin::TransactionId TransferMoney(MevaCoin::WalletLegacy& from, MevaCoin::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  MevaCoin::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(MevaCoin::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(MevaCoin::WalletLegacy& wallet);
  void GetOneBlockReward(MevaCoin::WalletLegacy& wallet, const MevaCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(MevaCoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const MevaCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new MevaCoin::WalletLegacy(m_currency, *newNode, m_logger));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new MevaCoin::WalletLegacy(m_currency, *bobNode, m_logger));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new MevaCoin::WalletLegacy(m_currency, *carolNode, m_logger));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(MevaCoin::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(MevaCoin::WalletLegacy& wallet, const MevaCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(MevaCoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const MevaCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<MevaCoin::WalletLegacyTransfer> trs;
  MevaCoin::WalletLegacyTransfer tr;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  MevaCoin::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, MevaCoin::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), MevaCoin::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  MevaCoin::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, MevaCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  MevaCoin::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, MevaCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  MevaCoin::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, MevaCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, initWithKeys) {
  MevaCoin::AccountKeys accountKeys;
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  MevaCoin::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, MevaCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public MevaCoin::IWalletLegacyObserver {
  public:
    ExternalTxChecker(MevaCoin::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(MevaCoin::TransactionId transactionId) override {
      MevaCoin::WalletLegacyTransaction txInfo;
  ...
  MevaCoin::WalletLegacy wallet(m_currency, *aliceNode, m_logger);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public MevaCoin::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  MevaCoin::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public MevaCoin::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(MevaCoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(MevaCoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  MevaCoin::AccountKeys accountKeys1;
  ...
  std::vector<MevaCoin::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  MevaCoin::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  MevaCoin::AccountKeys actualAccountKeys;
...
void generateWallet(MevaCoin::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  MevaCoin::Currency currency(MevaCoin::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  MevaCoin::WalletLegacy wallet(currency, node, m_logger);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  MevaCoin::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(MevaCoin::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = MevaCoin::NULL_SECRET_KEY;
