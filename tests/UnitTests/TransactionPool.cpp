...
class TransactionValidator : public MevaCoin::ITransactionValidator {
...
public:
  ...
  TestTransactionGenerator(const MevaCoin::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const MevaCoin::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const MevaCoin::Currency& currency, Logging::ILogger& logger) :
  ...
  public:
	...
	ICoreStub coreStub;
    MevaCoin::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = MevaCoin::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
